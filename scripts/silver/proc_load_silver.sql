	...
	BEGIN TRY
		...
		*, 
		...
		END prd_line, --Map product line codes to descriptive values
			...
			CAST(LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt) - 1 AS DATE) AS prd_end_dt -- Calculate end date as one day before the next start sate
